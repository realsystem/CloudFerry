...
class SSHChunksTransfer(driver_transporter.DriverTransporter):
    def transfer(self, data):
        ...
        runner_src = remote_runner.RemoteRunner(host_src, ssh_user_src)
        runner_dst = remote_runner.RemoteRunner(host_dst, ssh_user_dst)
        temp_dir_name = os.popen('mktemp -udt ssh_chunks_XXXX').read().rstrip()
        src_temp_dir = os.path.join(temp_dir_name, '')
        dst_temp_dir = os.path.join(temp_dir_name, '')

        attempts_count = self.cfg.migrate.retry
        part_size = self.cfg.migrate.ssh_chunk_size

        with files.RemoteDir(runner_src, temp_dir_name):
            with files.RemoteDir(runner_dst, temp_dir_name):
